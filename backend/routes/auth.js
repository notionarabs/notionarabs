...
router.get('/test-callback', async (req, res) => {
  try {
    ...
    console.log('MONGODB_URI format check:', process.env.MONGODB_URI?.startsWith('mongodb'));
    ...
    res.json({
      ...
      mongoUriFormat: process.env.MONGODB_URI?.startsWith('mongodb') ? 'correct' : 'incorrect'
    });
  } catch (error) {
