...
import { Layout, Zap, BookOpen, Users, Compass, Home, Calendar, Search, User as UserIcon, Command } from 'lucide-react';
            ...
            <nav className="space-y-2 mb-6" aria-label="Mobile navigation">
              ...
              {navItems.map((item) => (
                <Link
                  ...
                  href={item.href}
                  onClick={() => handleNavigation(item.href)}
                  className={`flex items-center justify-between py-4 px-4 rounded-2xl transition-all active:scale-95 ${activePage === item.key
