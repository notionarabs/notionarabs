          ...
          <nav className="space-y-2 mb-6" aria-label="Mobile navigation">
            ...
            {navItems.map((item) => (
              <Link
                ...
                href={item.href}
                onClick={() => handleNavigation(item.href)}
                className={`flex items-center justify-between py-3 px-4 rounded-xl transition-all active:scale-95 ${activePage === item.key
                  ? 'bg-primary-500/20 text-primary-300 dark:text-primary-400 font-bold border border-primary-500/30'
                  : 'text-gray-100 dark:text-dark-text-primary hover:bg-white/10 dark:hover:bg-dark-tertiary'
                  }`}
              >
                <span className="font-medium">{item.label}</span>
                {activePage === item.key ? (
                  <div className="w-2 h-2 rounded-full bg-primary-400 shadow-[0_0_10px_rgba(251,146,60,0.5)]"></div>
                ) : (
                  <svg className="w-5 h-5 opacity-50 rtl:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                )}
              </Link>
