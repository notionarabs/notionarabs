...
function ProjectCardSkeleton() {
    return (
        <div className="flex flex-col h-full bg-white dark:bg-dark-secondary border border-gray-200 dark:border-dark-card-border rounded-2xl overflow-hidden shadow-sm animate-pulse">
            <div className="relative w-full aspect-[16/9] bg-gray-200 dark:bg-dark-tertiary" />
            <div className="p-5 sm:p-7 flex flex-col flex-grow">
                <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gray-200 dark:bg-dark-tertiary" />
                    <div className="h-6 bg-gray-200 dark:bg-dark-tertiary rounded w-1/2" />
                </div>
                <div className="space-y-2 mb-6">
                    <div className="h-4 bg-gray-200 dark:bg-dark-tertiary rounded w-full" />
                    <div className="h-4 bg-gray-200 dark:bg-dark-tertiary rounded w-5/6" />
                </div>
                <div className="mt-auto">
                    <div className="h-10 bg-gray-200 dark:bg-dark-tertiary rounded-xl w-full" />
                </div>
            </div>
        </div>
    );
...
export default function FeaturedProjects() {
    ...
    const displayProjects = featuredSlugs.map(slug => {
        const p = projects.find(proj => proj.slug === slug);
        if (!p) return null;

        // Override featured project preview for home page specifically
        if (slug === 'yuyan-academy') {
            return {
                ...p,
